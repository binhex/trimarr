...
from trimarr.downloader import (
    _extract_from_tar,
    _extract_from_zip,
...
class TestGetAppDataDir:
    ...
    def test_windows_uses_localappdata(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("trimarr.downloader.platform.system", return_value="Windows"):
            result = get_app_data_dir()
...
class TestGetPlatformAsset:
    ...
    def test_linux_aarch64_raises(self) -> None:
        with (
            patch("trimarr.downloader.platform.system", return_value="Linux"),
            patch("trimarr.downloader.platform.machine", return_value="aarch64"),
    ...
    def test_unsupported_os_raises(self) -> None:
        with (
            patch("trimarr.downloader.platform.system", return_value="Darwin"),
            patch("trimarr.downloader.platform.machine", return_value="x86_64"),
            pytest.raises(RuntimeError, match="No pre-built mkvmerge binary"),
        ):
            _get_platform_asset()
...
class TestDownloadMkvmerge:
    ...
    def test_temp_binary_cleaned_up_when_atomic_replace_fails(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("trimarr.downloader.requests.get", return_value=_streaming_response(archive_bytes)),
            patch("trimarr.downloader.os.replace", side_effect=OSError("simulated replace failure")),
